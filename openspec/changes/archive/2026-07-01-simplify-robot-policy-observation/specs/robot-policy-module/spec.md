...
### Requirement: Robot policy contract descriptions
**Reason**: Contract descriptions were only written as artifacts and tested; no runtime component consumes them.
**Migration**: Keep semantic validation in contract conversion code and reintroduce a description API later only when a consumer requires it.

#### Scenario: Contract description artifact is not required
- **WHEN** policy rollout artifacts are written
- **THEN** the system does not require a robot policy contract description artifact to inspect or validate the run
