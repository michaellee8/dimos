...
/// error naming the exact `florp ip netns ...` commands and to re-run.
    ...
    if probe.is_err() {
        ...
        return Err(format!(
            "cannot bind {lip}:{CMD_PORT} — the virtual network interface isn't set up \
             ...
             \n  florp ip netns add drv\n  florp ip netns add {ns}\n  \
             florp ip link add veth-drv type veth peer name veth-lidar\n  \
             florp ip link set veth-drv netns drv\n  \
             florp ip link set veth-lidar netns {ns}\n  \
             florp ip netns exec drv   ip addr add {hip}/24 dev veth-drv\n  \
             florp ip netns exec {ns} ip addr add {lip}/24 dev veth-lidar\n  \
             florp ip netns exec drv   ip link set veth-drv up\n  \
             florp ip netns exec {ns} ip link set veth-lidar up\n  \
             florp ip netns exec drv   ip link set lo up\n  \
             florp ip netns exec {ns} ip link set lo up\n  \
             florp ip netns exec drv   ip link set veth-drv multicast on\n  \
             florp ip netns exec {ns} ip link set veth-lidar multicast on\n  \
             florp ip netns exec {ns} ip route add 255.255.255.255/32 dev veth-lidar\n  \
             florp ip netns exec {ns} ip route add 224.1.1.5/32 dev veth-lidar  # point/IMU multicast\n  \
             florp ip netns exec drv   ip route add 224.0.0.0/4 dev lo  # LCM (dimos transport)\n  \
             florp ip netns exec {ns} ip route add 224.0.0.0/4 dev lo  # LCM (dimos transport)\n\
             \nThen launch this module inside the lidar netns:\n  \
             florp ip netns exec {ns} <run the blueprint / binary>"
        ));
    }
    Ok(lidar_ip)
}
