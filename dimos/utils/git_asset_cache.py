...
"""Git-backed cache for robot description sources and other assets."""
...
DEFAULT_ROBOT_ASSET_CACHE_ROOT = Path.home() / ".cache" / "dimos" / "robot_assets"
...
class GitAssetCache:
    ...
    def _source_key(repo_url: str, ref: str) -> str:
        digest = sha256(f"{repo_url}\0{ref}".encode()).hexdigest()[:16]
        return digest
    ...
    def _clone_missing(self, repo_url: str, ref: str, checkout_path: Path) -> GitAssetCheckout:
        ...
        try:
            ...
            raise GitAssetCacheError(
                f"Failed to fetch robot asset source {repo_url!r} at ref {ref!r}: {exc}"
            ) from exc
    ...
    def _refresh_cached(self, repo_url: str, ref: str, checkout_path: Path) -> GitAssetCheckout:
        ...
        if self._is_dirty(repo):
            warnings.warn(
                f"Robot asset cache {checkout_path} has local changes; skipping upstream update.",
        ...
        try:
            ...
            warnings.warn(
                f"Could not update robot asset cache {checkout_path}; using cached checkout: {exc}",
                GitAssetCacheWarning,
                stacklevel=2,
            )
            return GitAssetCheckout(
