...
Each robot gets its own namespaced GO2Connection: topics like
``/robot0/lidar``, TF frames like ``robot0/...``, and RPC like
``robot0/go2connection/move``. Configure a single robot with
``-o robot0/go2connection.<field>=<value>``.
...
_ips = [ip.strip() for ip in (global_config.robot_ips or "").split(",") if ip.strip()]
