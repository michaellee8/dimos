...
- Instead of using `RerunBridge` in blueprints we always use `vis_module` which allows the CLI to control if its rerun or no-vis at all. Use `vis_module_with_selector` only for explicit selector-managed visualization sessions.
...
- The Rerun topic selector UI depends on Reflex through the visualization extra. It runs as a local selector API plus a `reflex run` subprocess from `dimos/visualization/rerun/reflex_selector_app`; keep generated Reflex artifacts such as `.web/` and `.reflex/` out of source edits. Keep the existing Rerun websocket server focused on viewer click/teleop events, not topic catalog APIs.
