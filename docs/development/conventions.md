...
- Instead of using `RerunBridge` in blueprints we always use `vis_module` which allows the CLI to control if its rerun or no-vis at all. Do not wire selector-specific visualization into ordinary blueprints; use `dimos topic monitor` as the attach-style selected visualization workflow.
...
- When adding visualization tools to a blueprint/autoconnect, instead of using RerunBridge or WebsocketVisModule directly we should always use `vis_module`, which right now should look something like `vis_module(viewer_backend=global_config.viewer, rerun_config={}),`. If runtime LCM topic selection is required, run `dimos topic monitor` after starting the stack rather than adding selector modules to the blueprint.
...
- The Rerun topic monitor/selector UI depends on Reflex through the visualization extra. It runs as a monitor-owned local selector API plus a `reflex run` subprocess from `dimos/visualization/rerun/reflex_selector_app`; keep generated Reflex artifacts such as `.web/`, `.states/`, and `.reflex/` out of source edits. Keep the existing Rerun websocket server focused on viewer click/teleop events, not topic catalog APIs.
